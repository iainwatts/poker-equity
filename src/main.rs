...
fn simulate_nine_players() {
    ...
    let hole_cards_for_players: Vec<Vec<&Card>> = (0..9).map(
        |player| (0..2).map(|card| deck.draw_nth(player * 2 + card + 5)).collect()
    ).collect();
    ...
    for (player, hole_cards) in hole_cards_for_players.iter().enumerate() {
        println!("Player {} was dealt {} {}", player, hole_cards[0], hole_cards[1]);

        let player_hand = get_scoring_hand_from_hole_cards_and_board(
            hole_cards, 
            &board_cards
        );
    ...
    let best_hands: Vec<&Hand> = player_hands.iter()
        .filter(|hand| hand == &one_best_hand).collect();
    println!("-----------------------");
...
fn get_scoring_hand_from_hole_cards_and_board<'a>(
    ...
    let possible_card_combos: Vec<Vec<&Card>> = all_cards.iter()
        .combinations(5)
        .map(|cards: Vec<&&Card>| cards.iter().map(|&&c| c).collect())
        .collect();

    let player_hand: Hand = possible_card_combos.iter()
        .map(|cards: &Vec<&Card>| Hand::new(cards.to_vec())).max().unwrap();
